...
# ===== HUMAN TRADER MENTALITY FILTER =====
def human_mentality_filter(df, bias):
    recent = df.tail(3)
    for idx, row in recent.iterrows():
        body = abs(row['c'] - row['o'])
        wick_top = row['h'] - max(row['c'], row['o'])
        wick_bottom = min(row['c'], row['o']) - row['l']
        if (wick_top > 2*body) or (wick_bottom > 2*body):
            return False
    if bias=="BUY" and df['c'].iloc[-1] > df['c'].iloc[-2]*1.015:
        return False
    if bias=="SELL" and df['c'].iloc[-1] < df['c'].iloc[-2]*0.985:
        return False
    return True
...
def generate_signal(symbol, tf):
    ...
    # Trend + EMA + Volume check
    if trend=="Uptrend" and price > ema50 and price <= sup*1.003 and vol > avg_vol*1.2:
        bias = "BUY"
        reason = f"Trend follow + EMA50 + Support + Volume spike + {trend}"
    elif trend=="Downtrend" and price < ema50 and price >= res*0.997 and vol > avg_vol*1.2:
        bias = "SELL"
        reason = f"Trend follow + EMA50 + Resistance + Volume spike + {trend}"

    if bias and not human_mentality_filter(df, bias):
        return None  # Skip if human-trader filter fails

    if not bias: return None

    fvg_up, fvg_down = fair_value_gap(df)
    pattern, pattern_dir = detect_chart_pattern(df)
    if bias=="BUY" and not ((fvg_up is not None) or (pattern_dir=="BUY")): return None
    if bias=="SELL" and not ((fvg_down is not None) or (pattern_dir=="SELL")): return None
    if pattern: reason += f" + Pattern: {pattern}"
    if (fvg_up or fvg_down): reason += " + FVG confirmed"
    ...
    return {"bias":bias,"entry":entry,"sl":sl,"tp":[tp2,tp3,tp4],
            ...
            "ob":ob,"vol":vol,"tf":tf,"reason":reason,"df":df,
...
async def main_loop():
    ...
    while True:
        ...
        await asyncio.sleep(5)  # 5-second polling for instant signal
